...
async function githubWorkflowDispatch(task) {
  ...
  // Resolve token from project_registry if project_name given; else default.
  ...
  if (project_name) {
    ...
    if (!token) return { ok: false, retryable: false, error: "GITHUB_PAT missing (no project_name to resolve secondary)" };
  }
...
async function pollGithubRunStatus(task) {
  ...
  // Resolve token via registry (project_name on this task is set by the dispatch handler)
  let token;
  if (project_name) {
    const proj = await getProject(project_name);
    if (proj.error) return { ok: false, retryable: false, error: proj.error };
    ...
    token = process.env.GITHUB_PAT;
    if (!token) return { ok: false, retryable: false, error: "GITHUB_PAT missing (no project_name on poll task)" };
  }
...
async function vercelRedeploy(task) {
  ...
  if (!project_name && !explicitId) {
    return { ok: false, retryable: false, error: "project_name or vercel_project_id required" };
  }
  ...
  if (project_name) {
    ...
    projectId = projectId || projectRow.vercel_project_id;
  }
  if (!projectId) return { ok: false, retryable: false, error: "vercel_project_id missing on registry row" };
  ...
  if (!r.ok) {
    if (r.status === 401 || r.status === 403) {
      return { ok: false, retryable: false, error: `vercel auth (${tokRes.env_name}): ${r.error}` };
    }
  ...
  const { data: depRow } = await supabase
    ...
    .insert({
      ...
      triggered_by: process.env.VERCEL_REGION || "worker",
      raw: { state: r.state, url: r.url, token_env: tokRes.env_name },
    })
    .select("id")
    .single();
...
async function vercelPromote(task) {
  ...
  if (!r.ok) {
    if (r.status === 401 || r.status === 403) return { ok: false, retryable: false, error: `vercel auth (${tokRes.env_name}): ${r.error}` };
...
async function githubGetRunStatus(task) {
  ...
  if (project_name) {
    ...
    token = process.env.GITHUB_PAT;
    if (!token) return { ok: false, retryable: false, error: "GITHUB_PAT missing (no project_name)" };
  }
...
export const HANDLERS = {
  ...
  // Phase D-multi — code_writer (PR-based code edits, all approval-gated upstream)
  ...
  // Phase E — verification + repo search
  grep_repo:                 grep_repo,
