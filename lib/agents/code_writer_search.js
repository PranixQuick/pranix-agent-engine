...
export async function grep_repo(task) {
  try {
    ...
    const pathFilter = input.path; // e.g. "app/" or "src/"
    const extFilter = input.extension; // e.g. "tsx"
    const includeContext = input.include_context !== false; // default true

    const token = tokenForRepo(repo);
    let allItems = [];
    for (let page = 1; page <= maxPages; page++) {
      // GitHub Code Search query syntax:
      //   <pattern> in:file repo:owner/name path:<path> extension:<ext>
    ...
    // For each hit, optionally fetch the file and find matching line numbers
    const matches = [];
    if (includeContext && allItems.length > 0) {
      // Limit content reads to top 20 results
