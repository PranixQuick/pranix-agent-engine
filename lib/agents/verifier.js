...
function safeError(e) {
  ...
  if (status && status >= 400 && status < 500 && status !== 429) {
    return { ok: false, retryable: false, error: `${e.message}${e.body?.error?.message ? " — " + e.body.error.message : ""}` };
  }
...
export async function verify_deployment(task) {
  ...
  try {
    ...
    // Note: passing empty teamId works as "personal scope" for some accounts; if the project lives on a personal account, omit teamId.
    const teamSuffix = proj.account_tier === "secondary" ? "" : teamQuery;

    // 1) Resolve target deployment ID
    let depId = input.vercel_deployment_id;
    let lookupVia = depId ? "direct_id" : null;

    if (!depId) {
      // List deployments for project, pick the most relevant
...
//
// Inputs:
//   { project_name, base_url? , timeout_ms? }
//     - if base_url omitted, uses project_registry.url
//     - runs all is_active smoke_test_definitions for project_name
